...
public class PageController {
	...
	{// 初始化代码块
		if(PageController.bigList==null) {
			PageController.bigList=DataOperate.getSelectAll("select * from scj");// 调用查询数据库的方法，返回所有的行
		}
		//获取总页数
	...
	public PageController(String sql) {//执行sql语句
		PageController.bigList=DataOperate.getSelectAll02(sql);//返回1列
	}
	...
	public boolean setInsert(String sql) {//判断sql 语句运行结果返回值是否为空
		PageController.bigList=DataOperate.getSelectAll02(sql);
		if(bigList.size()!=0) {
			return true;
		}else {
			return false ;
