...
public class PageController03 {
	...
	{// 初始化代码块
		if(PageController03.bigList==null) {
			PageController03.bigList=DataOperate.getSelectAll("select * from scj");// 调用查询数据库的方法，返回所有的行
		}
		//获取总页数
	...
	public PageController03() {}//无参构造方法
	
	public PageController03(String sql) {//执行sql语句
		PageController03.bigList=DataOperate.getSelectAll(sql);//返回1列
	}
	...
	public boolean setInsert(String sql) {//判断sql 语句运行结果返回值是否为空
		PageController03.bigList=DataOperate.getSelectAll02(sql);
		if(bigList.size()!=0) {
			return true;
		}else {
			return false ;
	...
	public PageController03(int curentPageIndex) {//构造方法设置当前页
