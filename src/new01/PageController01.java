...
public class PageController01 {
	...
	{// 初始化代码块
		if(PageController01.bigList==null) {
			PageController01.bigList=DataOperate.getSelectAll01("select * from scj");// 调用删除数据库的方法，返回所有的行
		}
		//获取总页数
	...
	public PageController01() {}//无参构造方法
	
	public PageController01(String sql) {//执行sql语句
		PageController01.bigList=DataOperate.getSelectAll02(sql);//返回1列
	}
	...
	public boolean setInsert(String sql) {//判断sql语句运行结果返回值是否为空
		PageController01.bigList=DataOperate.getSelectAll02(sql);
		if(bigList.size()!=0) {
			return true;
		}else {
			return false ;
	...
	public PageController01(int curentPageIndex) {//构造方法设置当前页
